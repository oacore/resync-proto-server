    ...
    def __init__(self, source):
        ...
        self.handlers = [
            ...
            (r"%s" % Source.RESOURCE_PATH, ResourcesHandler,
             dict(source=self.source)),
            (r"%s/(.*?)" % Source.RESOURCE_PATH, ResourceHandler,
             dict(source=self.source)),
...
class ResourceHandler(BaseRequestHandler):
    ...
    def get(self, basename):
        ...
        if resource is None:
            ...
            fs = os.path.join(self.source.folder, basename)
            payload = open(fs).read()
            (type, enc) = mimetypes.guess_type(fs)
            if enc is not None:
                self.set_header("Content-Encoding", enc)

            self.set_header("Content-Type", type)
            self.set_header("Content-Length", len(payload))
